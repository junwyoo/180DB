...
//                calc_out_turning_right = fann_run(turning_right_ann, input);
            ...
            case 3:
                ...
                break;
            case 4:
                calc_out_speed = fann_run(two_stair_ascent_ann, input);
                break;
            case 5:
                calc_out_speed = fann_run(two_stair_descent_ann, input);
                break;
            case 6:
...
//                turn = turn_left;*/
        ...
        else if(motion ==  4 ||  motion == 5) {
            printf("Motion: %s --> Speed: %s\t%s\n", activities_output[motion], two_staires_speeds[speed], turn_output[turn]);
        }
        else {
            printf("Motion: %s --> Speed: %s\t%s\n", activities_output[motion], standard_speeds[speed], turn_output[turn]);
        }
        //	} else
