...
//            turning[i] = turning_all[i];
        ...
        turning_file(turning_all, dot_text_turning, dimen);
        for (i = 0; i < ACTIVITIES; i++) {
...
//            printf("%s\n", dot_text_names[i+1]);
	    ...
	    train_network(dot_text_turning, dot_net_turning, TURN_FEATURES, NUM_TURNS);
	    train_network(dot_text_names[0], dot_net_name[0], FEATURES, ACTIVITIES);
        for (i = 0; i < ACTIVITIES; i++) {
            speeds = speeds_end[i] - speeds_start[i];
            train_network(dot_text_names[i+1], dot_net_name[i+1], FEATURES, speeds);
        }
        for (i = 0; i < FILES; i++) {
            for (j = 0; j < dimen[i]; j++) {
