...
void train_parameters(float ****classifiers, int **dimensions){
    ...
    for (i = 0; i < FILES; i++) {
        ...
        snprintf(input_files[i],
                 BUFF_SIZE,
                 "%s%s%s_%s.csv",main_path, paths[who], names[i], user_names[who]);
...
//        move_back(strides_t1[i], 100, count_t1[i]);
    ...
    // second dimension is stride start

    // count_t1/t2 array with number of strides in each file
    ...
    // data_length is number of samples per array
